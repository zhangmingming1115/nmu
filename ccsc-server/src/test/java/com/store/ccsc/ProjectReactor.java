...
public class ProjectReactor {
    ...
    private static final String GROUP_ID = "com.store.ccsc";
    private static final String ARTIFACT_ID = "ccsc";
    private static final String PACKAGE_NAME = "com.store.ccsc";
    private static final String TITLE = "民企清欠系统";
    ...
    public static void main(String[] args) {
        ...
        // ========== 配置，需要你手动修改 ==========
        String groupIdNew = "com.store.ccsc";
        String artifactIdNew = "ccsc";
        String packageNameNew = "com.store.ccsc";
        String titleNew = "民企清欠系统";
        ...
        // 写入文件
        String finalProjectBaseDirNew = projectBaseDirNew;
        files.forEach(file -> {
            // 如果是白名单的文件类型，不进行重写，直接拷贝
            String fileType = getFileType(file);
            if (WHITE_FILE_TYPES.contains(fileType)) {
                copyFile(file, projectBaseDir, finalProjectBaseDirNew, packageNameNew, artifactIdNew);
                return;
            }
            // 如果非白名单的文件类型，重写内容，在生成文件
            String content = replaceFileContent(file, groupIdNew, artifactIdNew, packageNameNew, titleNew);
            writeFile(file, content, projectBaseDir, finalProjectBaseDirNew, packageNameNew, artifactIdNew);
        });
